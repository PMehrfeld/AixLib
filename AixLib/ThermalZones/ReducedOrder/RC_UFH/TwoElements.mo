...
model TwoElements
  ...
  extends OneElement(AArray={ATotExt,ATotWin,AInt}, nPorts=1);
  ...
  BaseClasses.InteriorWall intWallRC(
    ...
    annotation (Placement(transformation(extent={{184,-50},{204,-28}})));

  BaseClasses.InteriorWall intTabsRC(
    n=1,
    RInt={0.0014985035952297956},
    CInt={1888941.1613616445},
    final T_start=T_start) annotation (Placement(transformation(
        extent={{-10,-10},{10,10}},
        rotation=0,
        origin={210,-140})));
  Modelica.Thermal.HeatTransfer.Components.Convection convIntTabs
...
protected
  ...
  Modelica.Blocks.Sources.Constant hConIntTabs_const(final k=20*1.7) if
                              ATotExt > 0
...
equation
  ...
  connect(convIntWall.solid, intWallRC.port_a)
    annotation (Line(
    points={{148,-40},{184,-40}},
    color={191,0,0},
    smooth=Smooth.None));
  connect(intWallRC.port_a, resExtWallIntWall.port_b)
    annotation (Line(
    points={{184,-40},{168,-40},{168,-106},{158,-106}},
    color={191,0,0},
    smooth=Smooth.None));
  ...
  elseif ATotExt > 0 and ATotWin > 0 and AInt > 0 then
    connect(thermSplitterIntGains.portOut[3], intWallRC.port_a)
      annotation (Line(
      points={{190,86},{162,86},{162,-40},{184,-40}},
      color={191,0,0},
      smooth=Smooth.None));
    connect(thermSplitterSolRad.portOut[3], intWallRC.port_a)
      annotation (
      Line(
      points={{-122,146},{-58,146},{-58,98},{160,98},{160,-40},{184,-40}},
      color={191,0,0},
      smooth=Smooth.None));
  end if;
  connect(resIntWallWin.port_b, intWallRC.port_a)
    annotation (Line(
    points={{94,-108},{124,-108},{124,-88},{174,-88},{174,-40},{184,-40}},
    color={191,0,0},
    smooth=Smooth.None));
  ...
  connect(intWallRC.port_a, intWallIndoorSurface)
    annotation (Line(points={{184,-40},{180,-40},{180,-82},{-120,-82},{-120,
          -180}},
    color={191,0,0}));
  connect(convIntWall.fluid, senTAir.port)
    annotation (Line(points={{128,-40},{76,-40},{76,0},{80,0}},
    color={191,0,0}));
  connect(hConIntTabs_const.y, convIntTabs.Gc) annotation (Line(points={{167.5,
          -155.2},{167.5,-158.6},{168,-158.6},{168,-150}}, color={0,0,127}));
  connect(intTabsRC.port_a, convIntTabs.solid) annotation (Line(points={{200,
          -140.909},{200,-140},{178,-140}}, color={191,0,0}));
  connect(thermSplitterSolRad.portOut[3], intTabsRC.port_a) annotation (Line(
        points={{-122,146},{106,146},{106,-92},{200,-92},{200,-140.909}}, color
        ={191,0,0}));
  connect(thermSplitterIntGains.portOut[3], intTabsRC.port_a) annotation (Line(
        points={{190,86},{106,86},{106,-92},{200,-92},{200,-140.909}}, color={
          191,0,0}));
  connect(intTabsRC.port_a, intWallIndoorSurface) annotation (Line(points={{200,
          -140.909},{200,-92},{-120,-92},{-120,-180}}, color={191,0,0}));
  connect(intTabsRC.port_a, resIntWallWin.port_b) annotation (Line(points={{200,
          -140.909},{200,-92},{94,-92},{94,-108}}, color={191,0,0}));
  connect(intTabsRC.port_a, resExtWallIntWall.port_b) annotation (Line(points={
          {200,-140.909},{200,-106},{158,-106}}, color={191,0,0}));
  connect(convIntTabs.fluid, senTAir.port) annotation (Line(points={{158,-140},
          {98,-140},{98,-40},{76,-40},{76,0},{80,0}}, color={191,0,0}));
  annotation (defaultComponentName="theZon",Diagram(coordinateSystem(
  preserveAspectRatio=false, extent={{-380,-180},{240,180}}), graphics={
  Polygon(
  ...
  Rectangle(
    extent={{146,-124},{238,-178}},
    lineColor={0,0,255},
    fillColor={215,215,215},
    fillPattern=FillPattern.Solid),
  Text(
    extent={{187,-167},{240,-176}},
    lineColor={0,0,255},
    fillColor={215,215,215},
    fillPattern=FillPattern.Solid,
          textString="Interior TABS")}),
  ...
  Icon(coordinateSystem(extent={{-380,-180},{240,180}},
