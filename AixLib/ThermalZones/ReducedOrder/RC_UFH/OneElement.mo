within AixLib.ThermalZones.ReducedOrder.RC_UFH;
model OneElement "Thermal Zone with one element for exterior walls"
  ...
  Modelica.Blocks.Interfaces.RealInput solRad[nOrientations](
    ...
    annotation (
    Placement(transformation(extent={{-420,120},{-380,160}}),
    iconTransformation(extent={{-400,140},{-380,160}})));
  ...
  Modelica.Blocks.Interfaces.RealInput QLat_flow(final unit="W") if
    ...
    annotation (Placement(transformation(extent={{-420,-150},{-380,-110}}),
        iconTransformation(extent={{-400,-130},{-380,-110}})));
  ...
  Modelica.Thermal.HeatTransfer.Interfaces.HeatPort_a extWall if ATotExt > 0
    ...
    extent={{-390,-50},{-370,-30}}), iconTransformation(extent={{-390,-50},{
            -370,-30}})));
  Modelica.Thermal.HeatTransfer.Interfaces.HeatPort_a window if ATotWin > 0
    "Ambient port for windows"
    annotation (Placement(transformation(extent={{-390,30},{-370,50}}),
    iconTransformation(extent={{-390,30},{-370,50}})));
  ...
  Modelica.Blocks.Interfaces.RealInput[Medium.nC] C_flow if use_C_flow
    ...
    annotation (Placement(transformation(extent={{-420,70},{-380,110}}), iconTransformation(extent={{-400,90},
            {-380,110}})));

  BaseClasses.ExteriorWall ExtTabsRC(
    n=1,
    RExt={0.0014985035952297956},
    RExtRem=0.10707292497619882,
    CExt={1888941.1613616445},
    final T_start=T_start) annotation (Placement(transformation(
        extent={{-10,-10},{10,10}},
        rotation=180,
        origin={-298,-10})));
  Modelica.Thermal.HeatTransfer.Components.Convection convExtTabs annotation (
      Placement(transformation(
        extent={{-10,-10},{10,10}},
        rotation=0,
        origin={-260,-8})));
  Modelica.Thermal.HeatTransfer.Sources.FixedTemperature TGround(T=286.15)
    annotation (Placement(transformation(
        extent={{-10,-10},{10,10}},
        rotation=0,
        origin={-338,-8})));
protected
  ...
  Modelica.Blocks.Sources.Constant hConExtTabs_const(final k=20*1.7) if
                              ATotExt > 0
...
equation
  ...
  connect(eRadSol.u, solRad)
    annotation (Line(points={{-207,146},{-214,146},{-214,140},{-400,140}},
    color={0,0,127}));
  ...
  connect(solRad, eConvSol.u)
    annotation (Line(
    points={{-400,140},{-226,140},{-226,124},{-207,124}},
    color={0,0,127},
    pattern=LinePattern.Dash));
  ...
  connect(mWat_flow.u, QLat_flow) annotation (Line(points={{-202,-90},{-232,-90},
          {-232,-130},{-400,-130}}, color={0,0,127}));
  connect(conQLat_flow.Q_flow, QLat_flow)
    annotation (Line(points={{-202,-120},{-232,-120},{-232,-130},{-400,-130}},
                                                       color={0,0,127}));
  connect(volMoiAir.C_flow, C_flow) annotation (Line(points={{-22,-22},{-52,-22},
          {-52,90},{-400,90}}, color={0,0,127}));
  connect(volAir.C_flow, C_flow) annotation (Line(points={{44,-22},{56,-22},{56,
          90},{-400,90}}, color={0,0,127}));
  connect(ExtTabsRC.port_a, convExtTabs.solid) annotation (Line(points={{-288,
          -9.09091},{-270,-9.09091},{-270,-8}}, color={191,0,0}));
  connect(hConExtTabs_const.y, convExtTabs.Gc) annotation (Line(points={{-258,
          11.5},{-258,6},{-260,6},{-260,2}}, color={0,0,127}));
  connect(extWall, extWallRC.port_b) annotation (Line(points={{-380,-40},{-178,
          -40}},                  color={191,0,0}));
  connect(ExtTabsRC.port_b, TGround.port) annotation (Line(points={{-308,
          -9.09091},{-308,-8},{-328,-8}}, color={191,0,0}));
  connect(thermSplitterIntGains.portOut[1], convExtTabs.solid)
    annotation (Line(points={{190,86},{-270,86},{-270,-8}}, color={191,0,0}));
  connect(thermSplitterSolRad.portOut[1], convExtTabs.solid) annotation (Line(
        points={{-122,146},{-86,146},{-86,86},{-270,86},{-270,-8}}, color={191,
          0,0}));
  connect(convExtTabs.fluid, senTAir.port) annotation (Line(points={{-250,-8},{
          -42,-8},{-42,28},{80,28},{80,0}}, color={191,0,0}));
  connect(resExtWallWin.port_b, convExtTabs.solid) annotation (Line(points={{
          -146,-3.55271e-15},{-194,-3.55271e-15},{-194,0},{-238,0},{-238,32},{
          -270,32},{-270,-8}}, color={191,0,0}));
  connect(convExtTabs.solid, extWallIndoorSurface) annotation (Line(points={{
          -270,-8},{-270,-10},{-274,-10},{-274,-140},{-160,-140},{-160,-180}},
        color={191,0,0}));
  annotation (defaultComponentName="theZon",Diagram(coordinateSystem(
  preserveAspectRatio=false, extent={{-380,-180},{240,180}},
  grid={2,2}),  graphics={
  Rectangle(
    extent={{-206,80},{-92,26}},
    lineColor={0,0,255},
  ...
  Rectangle(
    extent={{-360,28},{-242,-26}},
    lineColor={0,0,255},
    fillColor={215,215,215},
    fillPattern=FillPattern.Solid),
  Text(
    extent={{-359,25},{-306,16}},
    lineColor={0,0,255},
    fillColor={215,215,215},
    fillPattern=FillPattern.Solid,
          textString="Exterior TABS")}),
  Icon(coordinateSystem(preserveAspectRatio=false, extent={{-380,-180},{240,180}},
  grid={2,2}),
   graphics={
  Rectangle(
    extent={{-380,180},{240,-180}},
    lineColor={0,0,0},
    fillColor={215,215,215},
